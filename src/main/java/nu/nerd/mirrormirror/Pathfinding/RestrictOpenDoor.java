...
public class RestrictOpenDoor extends AbstractPathfinderGoal {


    public RestrictOpenDoor(LivingEntity baseEntity) {
        super(baseEntity);
    }
    ...
    public Object get() {
        try {
            ...
            Class c = NMSHelper.getNMSClassByName("PathfinderGoalRestrictOpenDoor");
            Constructor constructor = c.getConstructor(creature);
            return constructor.newInstance(creature.cast(getHandle()));
        } catch (Exception ex) {
            MirrorMirror.logger().warning("Error constructing PathfinderGoalRestrictOpenDoor");
            ex.printStackTrace();
            return null;
        }
    }
